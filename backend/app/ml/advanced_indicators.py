...
def money_flow_index(ohlcv: pd.DataFrame, length: int = 14) -> pd.Series:
    """MFI (Money Flow Index) — RSI'nin hacim ağırlıklı karşılığı: 0..100,
    50 üstü alım baskısı, 50 altı satım baskısı."""
    typical = (ohlcv["high"] + ohlcv["low"] + ohlcv["close"]) / 3
    raw_flow = typical * ohlcv["volume"]
    direction = np.sign(typical.diff().fillna(0.0))
    pos_flow = pd.Series(np.where(direction > 0, raw_flow, 0.0), index=ohlcv.index)
    neg_flow = pd.Series(np.where(direction < 0, raw_flow, 0.0), index=ohlcv.index)
    pos_sum = pos_flow.rolling(length).sum()
    neg_sum = neg_flow.rolling(length).sum().replace(0, float("nan"))
    mfr = pos_sum / neg_sum
...
def elder_force_index(ohlcv: pd.DataFrame, length: int = 13) -> pd.Series:
    ...
    (büyük hacimli hareketler daha güvenilir kabul edilir)."""
    raw = ohlcv["close"].diff() * ohlcv["volume"]
...
def realized_volatility_spread(close: pd.Series, short: int = 5, long: int = 20) -> pd.Series:
    """Kısa vadeli (`short` barlık) ile uzun vadeli (`long` barlık)
    gerçekleşen volatilite (getiri std'si) arasındaki fark — pozitifse
    volatilite YAKINDA ARTIYOR (rejim değişimi olabilir), negatifse
    volatilite YATIŞIYOR."""
...
def rolling_price_volume_correlation(ohlcv: pd.DataFrame, length: int = 20) -> pd.Series:
    """Fiyat getirisi ile hacim arasındaki kayan pencereli korelasyon —
    pozitifse hacim artışı fiyat yükselişiyle birlikte geliyor (sağlıklı
    trend), negatifse ayrışma var (olası zayıflama sinyali)."""
    returns = ohlcv["close"].pct_change()
    # `.clip(-1, 1)` MATEMATİKSEL sınırı zorlar — hacim (nadiren de olsa)
    # bir pencerede tamamen SABİT kalırsa (varyans ≈ 0), pandas'ın `.corr()`u
    # kayan nokta hassasiyeti yüzünden NaN yerine +-inf üretebiliyor
    # (üretimde/testte GÖZLENDİ: sabit hacimli sentetik veride ±inf,
    # XGBoost'un "inf/çok büyük değer" kontrolünü kırıp eğitimi çökertti).
    return returns.rolling(length).corr(ohlcv["volume"]).clip(-1, 1).fillna(0.0)
