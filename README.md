...
- [x] **Karlılık: Backtest artık çalışır durumda, gerçek veriyle ilk iki iyileştirme yapıldı** (OOM saga'sı çözülüp `POST /backtest/system/run` sorunsuz döndükten sonra kullanıcı isteği: "karlılığı artırmamız gerekiyor"):
  ...
  18. **Yeni özellik grubu eklendi (kullanıcı isteği: "kolay olanları ekleyip performansa bakalım", sonrasında yüksek korelasyonlu olanlar elenecek)**: Kullanıcının önerdiği 26 özellikten yeni veri kaynağı GEREKTİRMEYEN (mevcut OHLCV'den hesaplanabilen) ~18'i eklendi — `ALL_FEATURE_COLUMNS` 63'ten **80**'e çıktı:
      - **Eksik klasik göstergeler**: `mfi_norm` (MFI, RSI'nin hacim ağırlıklı hali), `williams_r_norm` (Williams %R), `elder_force_index_norm` (Elder's Force Index, z-skorlanmış), `keltner_dist_pct` (Keltner Channel — ATR tabanlı bant), `donchian_position` (Donchian Channel pozisyonu), `choppiness_index` (trend mi/yatay mı — `hurst_exponent`e benzer bir soruyu farklı formülle cevaplıyor, YÜKSEK KORELASYON adayı).
      - **Volatilite yapısı**: `parkinson_volatility` (high/low tabanlı volatilite tahmincisi, kapanış-tabanlıdan istatistiksel olarak daha verimli), `volatility_spread` (kısa vade - uzun vade gerçekleşen volatilite farkı, rejim değişimi sinyali).
      - **Zaman/takvim döngüsü**: `hour_sin`/`hour_cos`, `day_of_week_sin`/`day_of_week_cos` (döngüsel sin/cos kodlama — ham saat/gün sayısı DEĞİL, 23:00↔00:00 bitişikliğini modelin öğrenebilmesi için). `days_until_option_expiry` BİLEREK EKLENMEDİ (vadeli işlem takvimi gerektiriyor, "kolay" kategoride değil).
      - **Multi-timeframe derinleştirme**: `htf_4h_supertrend_trend`, `htf_1d_vwap_gap_pct` (mevcut `_HTF_SOURCE_COLUMNS` genel mekanizmasının DIŞINDA, yalnızca belirtilen üst-TF için tek bir ek gösterge — `_EXTRA_HTF_COLUMNS`), `htf_weekly_pivot_dist` (klasik pivot: bu haftanın pivotu ÖNCEKİ haftanın (H+L+C)/3'ü, `shift(1)` ile look-ahead önlenir).
      - **Etkileşim/türetilmiş**: `price_volume_correlation` (fiyat getirisi × hacim kayan korelasyonu), `bb_lower_upper_ratio` (Bollinger üst/alt bant oranı).
      - Eklenmeyen (yeni veri kaynağı/entegrasyon gerektirdiği için ERTELENDİ): emir defteri/akış/OI'nin ileri kalemleri (liquidation volume, long/short ratio — yeni borsa endpoint'leri), zincir-içi (on-chain) veriler (Glassnode/CryptoQuant benzeri genelde ücretli bir servis gerekir).
      - **Bulunan/düzeltilen 2 hata**: (1) `feature_snapshots` tablosu her yeni özellik için AYRI bir SQLAlchemy kolonuna sahip (dinamik/JSON değil) — yeni 14 kolon `FeatureSnapshot` modeline nullable Float olarak eklenmedi ise `record_feature_snapshot` `TypeError` ile patlıyordu; eklendi (4. tur, `_add_missing_columns` üretimde otomatik `ALTER TABLE` ile mevcut tabloya işleyecek). (2) `rolling_price_volume_correlation`: hacim bir pencerede TAMAMEN SABİT kalırsa (varyans≈0) pandas'ın `.corr()`u NaN yerine kayan nokta hassasiyeti yüzünden **+-inf** üretebiliyordu — sentetik bir testte (sabit hacimli fikstür) gerçekten YAKALANDI, XGBoost'un "inf/çok büyük değer" kontrolünü kırıp eğitimi çökertiyordu. Kaynakta (`advanced_indicators.py`) `.clip(-1, 1)` (korelasyonun matematiksel sınırı) eklenerek düzeltildi.
  19. **Yeni özelliklerle ölçüldü — SONUÇ AŞIRI UYUM UYARISI verdi, korelasyon eleme uygulandı**: `bash deploy/train-all.sh` + Backtest — xgboost `oos_balanced_acc` 0.421→**0.469** iyileşti, ama TAM SİSTEM backtest'i **352→351 işlem, %59,83 kazanma, +%9,56 PnL, %1,63 düşüş** verdi — önceki (80 özellik öncesi) turdan (523 işlem/+%13,34 PnL/%0,65 düşüş) PnL DÜŞTÜ, drawdown 2,5× ARTTI. İzole doğruluk artışı yine (LSTM vakasındaki gibi) tam sistem performansını GARANTİ ETMEDİ. `bash deploy/check-feature-correlations.sh` gerçek BTC verisiyle 63 özellik/1953 çift tarayıp **17 çift >=0.90 korelasyonlu** buldu — bunların 6'sı BUGÜN eklenen yeni özelliklerin ZATEN VAR OLAN özelliklerle örtüştüğünü gösterdi: `donchian_position`↔`price_position` (**1.0**), `bb_lower_upper_ratio`↔`bb_bandwidth_norm` (**1.0**), `williams_r_norm`↔`price_position`/`donchian_position` (0.90), `keltner_dist_pct`↔`rsi_norm`/`nwe_position` (0.97/0.94), `parkinson_volatility`↔`atr_pct` (0.94), `htf_1d_vwap_gap_pct`↔`htf_4h_ema_gap`/`htf_1d_rsi_norm`/`htf_weekly_pivot_dist` (0.93/0.93/0.92). Kalan 11 çift (ör. `stoch_rsi_k`↔`stoch_rsi_d`, `price_position`↔`bb_percent_b`) ÖNCEDEN VAR OLAN, kullanıcının önceki turlarda bilerek eklediği göstergeler arasında — bunlara DOKUNULMADI (kapsam dışı, ayrı bir onay gerektirir). Kullanıcı onayıyla bu 6 özellik (+ ilgili `advanced_indicators.py` fonksiyonları `williams_r`/`keltner_channel`/`donchian_channel_position`/`parkinson_volatility`, artık kullanılmadıkları için) koddan çıkarıldı — `ALL_FEATURE_COLUMNS` 80'den **74**'e indi.
  20. **Kullanıcı isteği: "eski özellikler arasında da gürültü varsa temizleyelim" — 5 ÖNCEDEN VAR OLAN özellik daha çıkarıldı**: 17 çiftin 6'sı tamamen ÖNCEDEN VAR OLAN (yeni eklenenlerle ilgisi olmayan) özellikler arasındaydı: `return_1`↔`candle_body_pct` (**1.0**), `price_position`↔`bb_percent_b` (0.908), `rsi_norm`↔`di_diff_norm` (0.923), `mavilim_gap`↔`vwap_gap_pct` (0.945), `return_3`↔`ha_body_pct` (0.923), `stoch_rsi_k`↔`stoch_rsi_d` (0.941). Her çiftte daha basit/köklü veya senin manuel trading'de bilerek eklediğin taraf TUTULDU: `candle_body_pct`/`bb_percent_b`/`di_diff_norm`/`vwap_gap_pct`/`ha_body_pct` ÇIKARILDI (`return_1`/`price_position`/`rsi_norm`/`mavilim_gap`/`return_3` tutuldu). **`stoch_rsi_k`/`stoch_rsi_d` BİLEREK dokunulmadı** — bu ikisi tasarım olarak korelasyonlu (D, K'nın yumuşatılmışı) ve kesişim (crossover) sinyali için BİRLİKTE gerekiyor, yüksek korelasyon burada gürültü değil kasıtlı bir tasarım. Ayrıca `return_3` (1. tur, veritabanında NOT NULL kolon) çıkarılamazdı — `_add_missing_columns` migration mekanizması kolon silme/nullable yapmayı desteklemiyor, bu yüzden `ha_body_pct` (daha sonraki, nullable bir kolon) tercih edildi. `ALL_FEATURE_COLUMNS` 74'ten **69**'a indi.
  21. **DOĞRULAMA (69 özellikle) — sonuç GERİLEDİ, sebep bulundu ve düzeltildi**: `bash deploy/train-all.sh` sonrası Backtest: **292 işlem, %54,79 kazanma, +%3,68 PnL, %0,91 düşüş** — hem 80-özellikli aradan (+%9,56) hem başlangıç zirvesinden (523 işlem/+%13,34/%0,65) NET bir düşüş, izole doğruluk (oos_balanced_acc=0.462) sağlıklı kalmasına rağmen. Üç ardışık turda (63→80→74→69 özellik) PnL sürekli düştü (13,34→9,56→3,68) — LSTM/çok-sembol derslerinin BİR DAHA doğrulandığı bir vaka. Revert etmeden önce kullanıcının önerisiyle (`"bu na göre labeling, sizing vb ayarlamaları tekrar yapsak ne olur"`) güven eşiği + Kelly + etiketleme YENİDEN tarandı — güven eşiği/Kelly TEK BAŞINA düşüşü telafi edemedi (en iyisi +%3,68 kaldı, mevcut 0.5/0.45 ile aynı), ama **etiketleme (horizon) taraması** asıl nedeni ortaya çıkardı: `horizon=8` (eski özellik setinde optimal) bu YENİ özellik setiyle artık çok zayıf (816 işlem/%50,12/+%0,63) — özellik seti değişince etiketleme hedefinin de yeniden kalibre edilmesi gerekiyormuş. `horizon=2/ATR=0.75` tam sistem backtest'inde en yüksek PnL'i verdi (335 işlem, %60,30 kazanma, **+%9,13** PnL, %1,29 düşüş) — `_ENSEMBLE_LABELING` güncellendi. Sıradaki adım: bu yeni etiketlemeyle yeniden eğitim + backtest, ardından güven eşiği/Kelly'nin BU kombinasyon için yeniden taranması (etiketleme değiştiği için eşikler de değişmiş olabilir).
