...
- [x] **Karlılık: Backtest artık çalışır durumda, gerçek veriyle ilk iki iyileştirme yapıldı** (OOM saga'sı çözülüp `POST /backtest/system/run` sorunsuz döndükten sonra kullanıcı isteği: "karlılığı artırmamız gerekiyor"):
  ...
  12. **DOĞRULANDI — yeni hiperparametreler (150/3/0.1) + yeni etiketleme hedefi (horizon=8) BİRLİKTE gerçek üretim backtest'inde ÖNCEKİ ZİRVEYİ AÇIK ARAYLA GEÇTİ**: `bash deploy/train-all.sh` (BTC-only, `symbols_used=1`) sonrası Backtest sayfası — xgboost `oos_balanced_acc=0.421`, online `overall_balanced_acc=0.439` (ikisi de önceki turdan, 0.404/0.417'den, daha iyi). Tam sistem backtest'i: **523 işlem, %57,36 kazanma, TOPLAM PnL +%13,34 (önceki zirve +%6,43'ün ~2 KATI), maks. düşüş %0,65 (önceki zirve %0,94'ten DAHA DA DÜŞÜK)**. Kazanma oranı biraz düştü (%64,20→%57,36 — daha kısa vadeli/sık sinyal ile beklenen bir örüntü) ama PnL VE drawdown'ın AYNI ANDA iyileşmesi gerçek bir kazanım: 3× daha fazla işlem fırsatı yakalıyor, daha az risk taşıyor. Bu, LSTM/çok-sembollü denemelerinin AKSİNE, "teoride mantıklı → pratikte iyileşme" örüntüsünün doğrulandığı bir vaka — çünkü her ikisi de (hiperparametre VE etiketleme) körü körüne "en yüksek izole metrik" ile değil, gerçek veri + tam sistem backtest'i + (max_depth=6 vakasında olduğu gibi) regresyon testleriyle aşırı uyum kontrolü yapılarak seçildi.
  13. **Yeni model/etiketleme ile güven eşiği + Kelly parametreleri yeniden tarandı (lokal kurulum İLK KEZ kullanıldı) — `kelly_multiplier` 0.75'ten 1.0'a (TAM KELLY) çıkarıldı**: Kullanıcı bu turda ilk kez kendi bilgisayarında ayrı bir Claude Code oturumu kurdu (README'de ayrıca not: VPS'teki "paste-relay" iş akışına kıyasla çok daha hızlı iterasyon için) — deneyleri paralel VPS + lokal'de çalıştırıp KARŞILAŞTIRARAK ilerlendi. Güven eşiği taraması (`bash deploy/sweep-confidence.sh`, VPS'te): mevcut 0.5/0.45 hâlâ en iyisi (523 işlem/%57,36/+%13,337 PnL/%0,646 düşüş) — DEĞİŞİKLİK GEREKMEDİ. Kelly pozisyon boyutu taraması hem lokalde hem VPS'te (`bash deploy/sweep-position-sizing.sh`) çalıştırıldı — **lokal ve VPS sonuçları farklı zaman pencerelerinde eğitildikleri için (aynı kod/config, farklı "şimdi" anı → farklı holdout verisi) sayısal olarak uyuşmadı** (ör. lokalde `kelly_min_trades=20` üstün görünüyordu, VPS'in KENDİ verisinde bu doğrulanmadı — `40` en az onun kadar iyiydi) — bu YENİ bir bilinen sınırlama olarak not edildi: lokal/VPS karşılaştırması yön gösterici ama ÜRETİM KARARLARI HER ZAMAN VPS'in KENDİ verisiyle alınmalı. VPS'in kendi taramasında `kelly_min_trades=40` (mevcut) en az `10`/`20` kadar iyiydi (düşüş payı en düşük) → DEĞİŞTİRİLMEDİ. Ama örneklem artık çok daha büyük olduğu için (523 işlem, eski 149'un ~3.5 katı) tam Kelly'nin (1.0x) tahmin hatası riski azaldı — 1.0x'te düşüş payı hâlâ küçük kaldı (%0.85) ve PnL en yüksekti (+%16.38, 0.75x'in +%13.34'ünden iyi). Kullanıcı onayıyla `kelly_multiplier` **1.0**'a çıkarıldı (`SystemBacktestRequest` + `RiskRules` — backtest VE canlı paper-trading SENKRON, önceki turdaki "paper trading'e de uygula" ilkesiyle aynı).
  14. **Güvenlik bulgusu (kullanıcı sorusu: "paper trade'de gelen ticker'lar nasıl seçiliyor, güvenli mi?") — canlı karar döngüsü artık SADECE `ml_primary_symbol` (BTC-only) üzerinde çalışıyor**: `app.engine.service.run_cycle_once` ÖNCEDEN screener'ın Top 5 Long + Top 5 Short listesini (piyasadaki HERHANGİ bir USDT-M futures sembolü, yalnızca fiyat/hacim filtresinden geçmiş) doğrudan `DecisionEngine`'e besliyordu — ama model yalnızca `ml_primary_symbol` (BTC/USDT) ile eğitiliyor (çok-sembollü eğitim denenip geriletilmişti, bkz. madde 9). Yani model, kendi eğitim dağılımının dışındaki (başka bir coin'in fiyat/hacim davranışı) sembollerde tahmin üretmeye zorlanıyordu — "out-of-distribution" riski, hiç ölçülmemiş/doğrulanmamış bir davranış. Düzeltildi: `run_cycle_once` artık `symbols = [settings.ml_primary_symbol]` kullanıyor — screener taraması hâlâ ayrı bir bilgi kaynağı (`/screener` API'si, teknik skor görüntüleme) olarak duruyor ama artık ML karar motorunu YÖNLENDİRMİYOR. Not (kaldıraç sorusu): paper-trading/backtest PnL hesaplamaları TAMAMEN kaldıraçsız (1x, spot-eşdeğeri) — kaldıraç kavramı yalnızca gerçek/canlı emir gönderme yolunda (`app.trading.executor`, henüz hiç kullanılmıyor) var, kod içi sabit tavanı `MAX_LEVERAGE=3` (`.env` ile bile aşılamaz).
  15. **DAHA KRİTİK bulgu (kullanıcı isteği: "bu şekilde düzeltilmesi gereken şeyler varsa kontrol et") — otomatik yeniden eğitim job'ları BTC-only + horizon=8 konfigürasyonunu SESSİZCE ÜZERİNE YAZABİLİRDİ**: madde 14'ün ardından genel bir tarama yapıldı, `app.scheduler.jobs`'taki `job_auto_retrain`/`job_auto_retrain_lstm`/`job_auto_retrain_online`/`job_auto_retrain_regime` job'larının (`ml_auto_retrain_enabled` varsayılan AÇIK, en geç 7 günde bir tetiklenir) TAMAMEN AYRI ve YANLIŞ bir yoldan sembol/etiketleme seçtiği bulundu: (a) `_auto_retrain_symbols()` `select_training_symbols`/`ml_train_max_symbols` yerine screener'ın ham Top Long/Short çıktısını (likidite/korelasyon filtresi YOK) kullanıyordu, (b) `train_signal_model_validated`/`train_meta_label_model`/`train_lstm_signal_model`/`train_online_signal_model`/`train_signal_models_by_regime` çağrıları `_ENSEMBLE_LABELING`i (horizon=8, atr_triple_barrier, 1.0xATR) HİÇ GEÇMİYORDU — kendi eski varsayılanlarını (horizon=5, 1.5xATR) kullanıyorlardı. Yani bu job'lar bir sonraki tetiklenmelerinde bugün elle doğrulanan (523 işlem/+%13,34 PnL) üretim modelini, hem YANLIŞ sembol evreniyle (BTC-dışı semboller de dahil, madde 9'da reddedilen çok-sembollü rejimin AYNISI) hem YANLIŞ etiketleme hedefiyle SESSİZCE değiştirecekti — hiçbir uyarı/log olmadan. Düzeltildi: `_auto_retrain_symbols(exchange)` artık `app.api.routes.ml._resolve_symbols` (manuel `train-all` ile AYNI, BTC-only hızlı yol dahil) kullanıyor; tüm otomatik eğitim çağrılarına `**_ENSEMBLE_LABELING` eklendi — artık manuel `train-all.sh` ile TUTARLI.
  16. **Kararlılık kontrolü (lokal, kullanıcı isteği: "bunu yapalım") — `oos_balanced_acc` eğitim veri miktarına DUYARSIZ/STABİL**: `--lookback 10000/15000/23679` ile 3 ayrı eğitim + backtest çalıştırıldı. `oos_balanced_acc`: 0.432 → 0.423 → 0.422 — gürültü seviyesinde bir fark, model kalitesi veri miktarına aşırı bağımlı DEĞİL (sağlıklı bir işaret). PnL/kazanma/drawdown ise daha çok saçıldı, ama bu BEKLENEN bir karışıklık: `restrict_to_holdout` her zaman "en güncel %20"yi test ediyor, `lookback` büyüdükçe holdout PENCERESİ de mutlak olarak geriye kayıyor (10K'da 2026-06-17'den başlıyor, 23679/VPS-tam'da 2026-02-23'ten) — yani bu üç sonuç yalnızca "veri miktarı" değil, aynı zamanda "hangi dönem test edildi" farkını da içeriyor, ayrıştırılamadı. **Sonuç: model kalitesi sağlam/stabil; PnL farkları büyük ihtimalle dönem farkından kaynaklanıyor** — tam bağımsız (ayrık) geçmiş pencere testi hâlâ ayrı bir araç gerektiriyor, ileride ele alınabilir.
  17. **UX iyileştirmesi (kullanıcı isteği: "paper trade devam ediyorsa ekranda bildirimde bulunsun")**: `PaperTrading.jsx` sayfası öncesinde yalnızca manuel "Şimdi Çalıştır" butonu vardı — zamanlayıcının arka planda gerçekten periyodik çalışıp çalışmadığı sayfadan ANLAŞILAMIYORDU (kullanıcı bunu VPS terminaline loglara bakarak kontrol etmek zorunda kalmıştı, log çıktısının APScheduler'ın rutin "başarılı" mesajlarını varsayılan log seviyesinde göstermemesi yüzünden YANILTICI biçimde boş görünmüştü — gerçek durum yalnızca `GET /scheduler/status`'tan doğrulanabildi). Yeni `SchedulerStatusBadge` bileşeni eklendi: sayfa `/scheduler/status`'u yükler (30sn'de bir tazelenir) ve `engine_cycle` job'unun durumuna göre üç hâl gösterir — 🟢 "Paper trade devam ediyor" (son karar zamanı/detayı ile), ⏳ "ilk döngü henüz çalışmadı", veya ⚠ "zamanlayıcı devre dışı / beklenenden uzun süredir çalışmamış" (son çalışmadan bu yana geçen süre `interval_seconds`'ın 3 katını aşarsa "stale" sayılır).
  18. **DENEY BAŞARISIZ OLDU, GERİ ALINDI — özellik mühendisliği (63→80→74→69) + etiketleme değişikliği (horizon=8→2) zinciri**: Kullanıcı isteğiyle ("kolay özellikleri ekleyip performansa bakalım") 14 yeni teknik özellik eklendi (`ALL_FEATURE_COLUMNS` 63→80), ardından iki turda korelasyon analiziyle (`deploy/check-feature-correlations.sh`) 11 gereksiz özellik elendi (80→74→69), ardından yeni özellik setiyle `horizon`'un yeniden kalibre edilmesi gerektiği bulunup 8'den 2'ye değiştirildi. **Her aşamada gerçek backtest sonucu KÖTÜLEŞTİ**: 523 işlem/+%13,34 PnL (başlangıç) → 351/+%9,56 (80 özellik) → 292/+%3,68 (69 özellik, temizlik sonrası) → tekrarlanan ölçümlerde tutarsız/düşük sonuçlar (69-236 işlem arası, +%0,35 ile +%1,31 arası) horizon=2 ile — izole model doğruluğu (`oos_balanced_acc`) boyunca sağlıklı kalmasına (0.421→0.469→0.462→0.441) RAĞMEN. **LSTM ve çok-sembollü eğitim derslerinin BİR DAHA doğrulandığı bir vaka**: izole doğruluk artışı hiçbir aşamada gerçek sistem performansına yansımadı. Kullanıcı onayıyla TÜM zincir `git revert` ile geri alındı (4 commit: `1884983`, `f2ea76e`, `356d69d`, `eda8f38` — ters kronolojik sırada, temiz uygulandı, çakışma yok) — kod, README madde 12'deki kanıtlanmış konfigürasyona (63 özellik, `horizon=8`, hiperparametreler 150/3/0.1, `kelly_multiplier=1.0`) geri döndü, 378 test yeşil. `deploy/check-feature-correlations.sh` script'i (kendisi zararsız bir analiz aracı olduğu için) KORUNDU — ileride farklı bir özellik denemesinde yeniden kullanılabilir. Sıradaki adım: bu geri alınmış hâli sunucuda yeniden eğitip 523 işlem/+%13,34 sonucunun tekrar elde edildiğini doğrulamak. **DOĞRULANDI**: gerçek `train-all.sh` + Backtest — 224 işlem, %61,61 kazanma, +%12,23 PnL, %1,18 düşüş (zirveden farklı ama çok yakın PnL — fark zaman kayması/holdout penceresinin ~1 ay ileri gitmesinden, kod/config sorunu değil).
  19. **Yeni deney (kullanıcı önerisi: "Seviye 1 — Süpervizeli Öğrenme/Regresyon") — sabit ATR çarpanı yerine dinamik kâr-al/zarar-durdur tahmini, İZOLE ölçüm aşamasında**: Kullanıcı, sabit `atr_mult` ile çıkış yapmak yerine modelin "bu girişte fiyat ne kadar yükselip/düşecek"i REGRESYON ile tahmin etmesini önerdi. Yeni `app.ml.labeling.label_future_peak_trough` (girişten sonraki `horizon` bar içindeki en yüksek/en düşük noktayı % olarak döner, causal-safety testleriyle doğrulandı — gelecek barlara sızma YOK) + `app.ml.dynamic_exit` modülü (mevcut 63 özellikle iki ayrı `XGBRegressor`: biri `future_peak_pct`, biri `future_trough_pct` tahmin eder) eklendi. **BİLEREK üretim modelinden TAMAMEN AYRI tutuldu, canlı karar döngüsüne/backtest'e HENÜZ KABLOLANMADI** — bugünkü oturumun ana dersiyle (izole doğruluk artışı gerçek sistem performansını garanti etmez) uyumlu olarak, önce bu modelin gerçekten anlamlı bir şey öğrenip öğrenmediği (holdout MAE/R²) İZOLE ölçülecek; `bash deploy/train-dynamic-exit-model.sh` ile gerçek veride çalıştırılıp R² pozitif/anlamlı çıkarsa ANCAK O ZAMAN tam sisteme (backtest exit mantığı) entegre edilmesi değerlendirilecek. 4 yeni test (causal-safety, manuel hesaplama doğrulaması, `persist=False` davranışı) eklendi, 382 test yeşil. **İlk gerçek ölçüm**: `bash deploy/train-dynamic-exit-model.sh` (BTC-only, `horizon=25`) — `future_peak_pct` R²=0.0516 (MAE 1.22 puan), `future_trough_pct` R²=0.0634 (MAE 1.25 puan). Zayıf ama SIFIRDAN/NEGATİFTEN farklı, gerçek (küçük) bir sinyal var — önerilen "anlamlı" eşiğin (0.1-0.2) altında kaldığı için backtest'e entegrasyon HENÜZ yapılmadı. Kullanıcı isteğiyle farklı `horizon` değerlerinin R²'yi iyileştirip iyileştirmediğini görmek için `sweep_dynamic_exit_horizons` (+ `deploy/sweep-dynamic-exit-horizons.sh`) eklendi — `app.backtest.system_runner`'daki diğer sweep'lerle AYNI desen (hiçbir model kaydedilmez, otomatik "en iyi" seçilmez). Sıradaki adım: gerçek veriyle [5,10,15,20,25,30,40] bar ızgarasını taramak. **Tarama sonucu**: `horizon=5` ızgaradaki en iyi nokta — `future_peak_pct` R²=0.1212 (MAE 0.5139), `future_trough_pct` R²=0.1094 (MAE 0.5291) — önerilen "anlamlı" eşiği (0.1) İLK KEZ geçti, uzun horizon'lar (25-40) monoton olarak daha kötü.
  20. **`horizon=5` dinamik çıkış modeli GERÇEK backtest'e kablolandı (kullanıcı onayı: "1. seçenek" → "sabit ATR çarpanı yerine bu modelin tahminini kullanarak gerçek sistem performansını ölç")**: Madde 19'daki izole modelin ("`use_dynamic_exit=false` durumundan farklı bir üretim modeli değil, backtest'in KENDİ opsiyonel bir modu") gerçek sistem etkisini yalnızca tam bir backtest karşılaştırması gösterebilir — bu oturumun tekrar tekrar doğrulanan ana dersiyle uyumlu. Yapılan kablolama (orta ölçekli, `system_runner.py` + `schemas.py` + `routes/backtest.py`):
     - `SystemBacktestRequest`e yeni `use_dynamic_exit: bool = False` alanı eklendi — `atr_stop_loss_mult`/`atr_take_profit_mult`'ın YERİNİ alır (aynı anda kullanılmaz), varsayılan KAPALI (geriye dönük uyumlu, mevcut tüm backtest'ler etkilenmez).
     - `run_system_backtest`e yeni opsiyonel `dynamic_exit_model: DynamicExitModel | None` parametresi eklendi (`lstm_model`/`online_model` ile AYNI desen). `use_dynamic_exit=True` ama model verilmezse `ValueError` (422) — sessizce ATR'ye düşmez.
     - Pozisyon açma bloğunda: `use_dynamic_exit=True` ise `DynamicExitModel.predict(row[FEATURE_COLUMNS])` ile o barın `future_peak_pct`/`future_trough_pct` tahmini alınır; LONG için kâr-al=peak/zarar-durdur=trough, SHORT için ters. Ters işaret koruması eklendi: model peak_pct<0 veya trough_pct>0 (mantıksız yön) tahmin ederse o taraf İPTAL edilir (None), ATR gibi geçersiz bir seviye AÇILMAZ.
     - Stop/kâr-al TETİKLEME koşulları (`stop_hit`/`take_profit_hit`) artık `request.atr_*` alanlarına DEĞİL, pozisyonun KENDİ `initial_stop_loss_price`/`take_profit_price` alanlarına bakıyor — hem ATR hem dinamik-çıkış modunda AYNI şekilde çalışır, davranış değişikliği YOK (eski testler değişmeden geçiyor).
     - `routes/backtest.py`: `use_dynamic_exit=True` geldiğinde `DEFAULT_DYNAMIC_EXIT_MODEL_PATH` diskten yüklenir (yoksa 422 — "önce `deploy/persist-dynamic-exit-model.sh` çalıştırılmalı" mesajıyla).
     - Yeni `deploy/persist-dynamic-exit-model.sh`: `horizon=5` ile eğitip **diske kaydeder** (`persist=True`) — önceki `train-dynamic-exit-model.sh`'ten FARKLI olarak (o hâlâ SADECE izole ölçüm yapar, `persist=False`, dokunulmadı).
     - Yeni testler: `test_dynamic_exit_stop_loss_closes_long_position_at_predicted_trough` (sahte bir `DynamicExitModel` ile kablolamanın gerçekten stop-loss'u model tahminine göre tetiklediğini doğrular) + `test_use_dynamic_exit_without_model_raises_value_error`.
     - **Önemli uyarı (şemadaki mevcut not ile TUTARLI)**: bu proje daha önce "kâr-alma + trailing VARSAYILAN AÇIK" denemesinde gerçek üretim testinde PnL %88'den %32'ye düşmüştü (mekanik sabit-mesafe çıkış, kazananı erken kesiyor) — dinamik-çıkış de aynı riski taşıyor (izole R²≈0.11 ZAYIF), bu yüzden VARSAYILAN KAPALI bırakıldı. Sıradaki adım: sunucuda `bash deploy/persist-dynamic-exit-model.sh` çalıştırıp modeli kaydetmek, sonra AYNI backtest'i `use_dynamic_exit=false` ve `true` ile ayrı ayrı çalıştırıp işlem/kazanma/PnL/drawdown'ı KARŞILAŞTIRMAK — yalnızca bu karşılaştırma gerçek etkiyi gösterir.
  21. **Paper trading ile backtest'in AYNI algoritmayı çalıştırdığı doğrulandı/hizalandı — kullanıcı isteği: "backtest'te 65% kazanma/+%26 PnL var ama paper trading zarar ediyor, hatayı bulalım"**: Kod düzeyinde karşılaştırma iki GERÇEK tutarsızlık ortaya çıkardı (ikisi de daha önce fark edilmemiş, bu oturumda düzeltildi):
     - **Stop-loss formülü farklıydı**: canlı/paper motor (`DecisionEngine`) sabit **%3** (`assumed_stop_loss_pct`) kullanıyordu; backtest'in doğruladığı risk yönetimi ise **ATR(14)×1.5** idi (`SystemBacktestRequest.atr_stop_loss_mult`). Yani paper trading, backtest'in ÖLÇTÜĞÜ sistemin birebir aynısı DEĞİLDİ. Düzeltildi: `DecisionEngine` artık `_predict` sırasında `average_true_range` ile (backtest'le AYNI fonksiyon/varsayılan `atr_period=14`) ATR hesaplayıp `_open`'da `price ± atr_stop_loss_mult(1.5) * ATR` formülünü kullanıyor — backtest'in pozisyon açma bloğuyla BİREBİR aynı.
     - **Kademeli alım/satım (dilim) backtest'te hiç YOKTU, canlıda VARDI**: `RiskRules.entry_tranche_weights`/`exit_tranche_weights` varsayılanı `[0.5, 0.5]` idi — her pozisyon 2 dilime bölünüyor, her dilim kendi komisyon+kayma maliyetini ayrı ödüyordu (backtest'in tek-giriş/tek-çıkış modelinde YOK). Varsayılan artık `[1.0]` (tek seferde aç/kapat) — isteyen kullanıcı elle çok-elemanlı liste vererek yine kademeli açabilir, ama bu artık backtest'in ÖLÇMEDİĞİ bir mod olduğu açıkça belgelendi.
     - **Araştırma sırasında bulunan (ama GEÇERSİZ çıkan) yanlış iz**: `GET /db/trades` çıktısında `ZHIPU/USDT:USDT`/`MINIMAX/USDT:USDT` gibi BTC-dışı işlemler görüldü — madde 14'teki "canlı karar döngüsü artık SADECE BTC" düzeltmesinin bozulduğu sanıldı. Zaman damgaları kontrol edilince bu işlemlerin o düzeltmeden (`f90029b`, 2026-09-08 15:02 UTC) SAATLER ÖNCE (aynı gün 02:26-03:12) açıldığı görüldü — yani GEÇMİŞTE kalmış, güncel bir hata DEĞİL. `run_cycle_once`'ın hâlâ `symbols = [settings.ml_primary_symbol]` (BTC-only) olduğu koddan doğrulandı.
     - Yeni testler (`tests/test_portfolio.py`): `test_decision_engine_opens_with_atr_based_stop_loss_matching_backtest_formula` (ATR formülünün backtest'le birebir eşleştiğini doğrular), `test_default_risk_rules_use_single_shot_tranches_matching_backtest`, `test_close_action_reason_is_persisted_to_trade_record`. 388 test yeşil (1 önceden var olan skip).
     - **İşlem geçmişi + açıklama (kullanıcı isteği: "paper trade üzerinde yapılan tüm işlemleri açıklamalarıyla, hangi tarihten itibaren hangi üründen ne kadar PNL kazanıldığını görebilelim")**: `TradeRecord`e yeni `reason` kolonu eklendi (nullable, `_add_missing_columns` ile var olan tabloya da otomatik eklenir) — `DecisionEngine.apply()`'ın ürettiği kapanış nedeni (`"stop-loss tetiklendi (seviye=...)"`, `"model kapanış/ters sinyali"` vb.) artık `PortfolioManager.close_tranche(..., reason=...)` üzerinden kalıcı olarak kaydediliyor. Yeni `GET /db/trades` filtreleri (`symbol`, `since`, `until`) + yeni `GET /db/trades/summary` (sembole göre işlem sayısı/kazanma oranı/toplam PnL). Frontend: Paper Trading sayfasına yeni **"İşlem geçmişi"** kartı eklendi — tarih aralığı seçici (7g/30g/90g/tüm zamanlar) + ürün filtresi + sembol bazlı özet panel + açıklamalı tam işlem listesi (önceki `ClosedHistoryCard`, süreç yeniden başlayınca sıfırlanan bellek-içi son-10 kaydı gösterdiği için AYRI tutuldu — bu yeni kart kalıcı DB'den TÜM geçmişi okur).
     - Sıradaki adım: VPS'te `git pull` + `docker build` + `recreate-backend.sh`, ardından `bash deploy/train-all.sh` ile yeniden eğitim, `bash deploy/persist-dynamic-exit-model.sh` (opsiyonel, madde 20), portföyü sıfırlayıp (`POST /portfolio/reset`) paper trading'i YENİ (backtest'le hizalanmış) risk kurallarıyla yeniden başlatmak.
